...
public class ParaCorrMap implements Runnable {
    ...
    public void run() {
      ...
      private int calFullSpread(ArrayList<ArrayList<ArrayList<Integer>>> degreeSets) {
        
          int totalNum = 1;
        for (int i = 0; i < megaSize.size(); i++) {
            ArrayList<ArrayList<Integer>> calset = fullTransOrder.get(i).get(megaSize.get(i));
            if (calset == null) {
                return 0;
            }
      ...
      HashMap<Integer, Integer> totalFullNum = new HashMap<>();
 //     ArrayList<ArrayList<ArrayList<Integer>>> degreeSets 
      HashMap<Integer, ArrayList<ArrayList<ArrayList<Integer>>>> totalSpeard = new HashMap<>();
    private HashMap<ArrayList<ArrayList<Integer>>, Integer> produceCorrMap() {
        ...
        for (int i = 0; i < thresh; i++) {
            ArrayList<ArrayList<Integer>> arr = genTrans(i, length);
            megaSize.put(i, arr);
        }

        for (int j = 0; j < sorted.get(0).getKey().size(); j++) {
            HashMap<Integer, ArrayList<ArrayList<Integer>>> transOrders = new HashMap<>();
            length = sorted.get(0).getKey().get(j).size();
            for (int i = 0; i < thresh; i++) {
                ArrayList<ArrayList<Integer>> arr = genTrans(i, length);
                transOrders.put(i, arr);
            }
            fullTransOrder.add(transOrders);
        }
        ...
        while (iteration < 2 && checkAvaCoutsEmpty()) {
...
//        tableCorrelationDistribution
    ...
    HashMap<Integer, ArrayList<ArrayList<Integer>>> megaSize = new HashMap<>();
    ArrayList<HashMap<Integer, ArrayList<ArrayList<Integer>>>> fullTransOrder = new ArrayList<>();

    private void mappingAllValueAesc(HashMap<ArrayList<ArrayList<Integer>>, Integer> result) {
        iteration++;

        System.out.println("endTrans");
        for (int i = 0; !avaCounts.isEmpty() && i < sorted.size()
                && checkAvaCoutsEmpty() && sorted.size() > 0; i++) {
            ...
            if (iteration != 1) {
                updateValue( entry.getKey(), result, value);
            }
            if (iteration == 1) {
                updateValueZero( entry.getKey(), result, value);
            }
            //   }
        }

        System.err.println(this.totalSum);
    }
    int thresh = 5;

    private void updateValue(ArrayList<ArrayList<Integer>> key, HashMap<ArrayList<ArrayList<Integer>>, Integer> result, int value) {
        ...
        for (int k = 0; k < thresh; k++) {
            ...
            ArrayList<ArrayList<Integer>> pairs  = this.megaSize.get(k);
            for (int j = 0; j < pairs.size(); j++) {
                if (budget == 0) {
                    return;
                }
                degreeSets.clear();
                ArrayList<Integer> singleSpread = pairs.get(j);
                boolean flag = true;
                for (int i=0; i<singleSpread.size();i++){
                    ArrayList<ArrayList<Integer>> singleAvas = new ArrayList<>();  
                    ArrayList<ArrayList<Integer>> singleThread=
                        this.fullTransOrder.get(i).get(singleSpread.get(i));
                    for (ArrayList<Integer> singleDegree : singleThread){
                        
                        singleAvas.add(singleDegree);
                    }
                }
    ...
    private void oldupdateValue(HashMap<Integer, ArrayList<ArrayList<Integer>>> transOrders, ArrayList<ArrayList<Integer>> key, HashMap<ArrayList<ArrayList<Integer>>, Integer> result, int value) {
        ArrayList<ArrayList<ArrayList<Integer>>> degreeSets = new ArrayList<>();
        ArrayList<ArrayList<Integer>> calDegrees = new ArrayList<>();
    ...
    private void updateValueZero( ArrayList<ArrayList<Integer>> key, HashMap<ArrayList<ArrayList<Integer>>, Integer> result, int value) {
    ...
    private void updateIndividualValue(ArrayList<ArrayList<Integer>> calDegrees, HashMap<ArrayList<ArrayList<Integer>>, Integer> result, int value) {
    ...
    private boolean checkSocial(ArrayList<ArrayList<Integer>> calDegrees) {
        return !eveNum
                || (eveNum && (this.sumVector(calDegrees.get(0)) < this.sumVector(calDegrees.get(1))
                || (calDegrees.get(0) == calDegrees.get(1) && calDegrees.get(0).get(2) == 1)));
    ...
    private void calCandidateDegrees(ArrayList<ArrayList<Integer>> calDegrees, int curNum, int totalNum, ArrayList<ArrayList<ArrayList<Integer>>> degreeSets) {
        for (int h = 0; h < degreeSets.size(); h++) {
            int index = curNum / (totalNum / degreeSets.get(h).size());
    ...
    private void processCalDegrees(ArrayList<ArrayList<Integer>> calDegrees, int value, ArrayList<ArrayList<Integer>> key, HashMap<ArrayList<ArrayList<Integer>>, Integer> result) {
        ...
        if (!this.boundTrash.contains(calDegrees) && checkSocial(calDegrees)) {
            updateIndividualValue(calDegrees, result, value);
            value = this.budget;
        } else {
        ...
        while (starter < value && this.checkAvaCoutsEmpty() && permutation.get(0).size() > 0) {
            ...
            if (val > 0) {
                if (!this.boundTrash.contains(preVdegrees)) {
                    int bound = findUniqBound(preVdegrees);
