...
public class ParaKeyIdAssign implements Runnable {
    ...
    ParaKeyIdAssign(HashMap<String, ArrayList<ArrayList<Integer>>> assignIDs,
            Map.Entry<String, HashMap<ArrayList<ArrayList<Integer>>, Integer>> entry,
            HashMap<String, HashMap<ArrayList<ArrayList<Integer>>, Integer>> scaledCorrelationDistribution,
            HashMap<String, ArrayList<ComKey>> avaMaps,
            HashMap<String, HashMap<ArrayList<ArrayList<Integer>>, ArrayList<Integer>>> referencingIDs,
    ...
    void newAssigning() {
        ...
        this.assignIDs.put(referTable, new ArrayList<ArrayList<Integer>>());
        //this.assignIDs.get(referTable).add(dump);
        ...
        for (Map.Entry<ArrayList<ArrayList<Integer>>, Integer> entry2 : entry.getValue().entrySet()) {
            ...
            if (entry2.getKey().size() == 1) {
                ...
                twoKeyGen(entry2, mmap);
            } else {
                threeKeyGen(entry2, mmap);
    ...
    private void oneKeyGen(Entry<ArrayList<ArrayList<Integer>>, Integer> entry2, HashMap<ArrayList<ArrayList<Integer>>, ArrayList<Integer>> mmap
    ) {
        int k = 0;
        ...
        for (int i = 0; i < count / queues.length * queues.length; i++) {
            ...
            this.assignIDs.get(this.referTable).add(tmp);
    ...
    private void twoKeyGen(Entry<ArrayList<ArrayList<Integer>>, Integer> entry2, HashMap<ArrayList<ArrayList<Integer>>, ArrayList<Integer>> mmap
    ) {
        int count = entry2.getValue();
        //     int k = 0;
        ...
        for (int j = 0; j < firstCols.length; j++) {
            for (int i = 0; i < firstCols[j]; i++) {
                ...
                this.assignIDs.get(this.referTable).add(temp);
                c2++;
                c2 = c2 % q2Len;
                // ids.put(id, temp);
                mmap.get(entry2.getKey()).add(id);
    ...
    private void threeKeyGen(Entry<ArrayList<ArrayList<Integer>>, Integer> entry2, HashMap<ArrayList<ArrayList<Integer>>, ArrayList<Integer>> mmap
    ) {
        ...
        while (!queue.isEmpty()) {
            mmap.get(entry2.getKey()).add(id);
            this.assignIDs.get(this.referTable).add(queue.poll());
            id++;
        }
    }
